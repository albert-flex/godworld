  ...
  <div class="socialpage">
    <main>
      ...
      <div style="display: flex">
        <a-image
          :width="200"
          :height="200"
          :src="FetchImage(LIB_SOCIAL_AVATOR, social.id)"
        ...
        />
        <div class="info">
          <h2>负责人: {{ social.masterName }}</h2>
          <h2>创建日期: {{ social.createTime }}</h2>
          <h2>最新活动: {{ social.newestAct }}</h2>
          <h2>最新公告: {{ social.newestAnnounce }}</h2>
        </div>
      </div>
      <div>
        <h2>签名:</h2>
      ...
      <a-tabs v-model:activeKey="activeKey">
        <a-tab-pane key="1" tab="活动">
          ...
          <div v-for="item in acts" :key="item.id"></div>
        </a-tab-pane>
        <a-tab-pane key="2" tab="公告" force-render>
          <a-list
            item-layout="vertical"
            :pagination="announcePagi"
            :data-source="anns"
          >
            <template #renderItem="{ item }">
              <a-list-item key="item.id">
    ...
    <aside>
      ...
      <a-divider
        ><h2>
          负责人:
          <a-button type="ghost" @click="toAuthor(members.master.id)">{{
            members.master.name
          }}</a-button>
        </h2></a-divider
      >
      ...
      <a-list
        item-layout="vertical"
        :pagination="adminPagi"
        :data-source="members.admins"
      >
        <template #renderItem="{ item }">
          <a-list-item key="item.id">
            <template #extra>
              <a-image
                :width="150"
                :src="FetchImage(LIB_BOOK_IMAGE, item.presentBookId)"
                fallback="data:image/png;base64,"
              />
            </template>
            <a-list-item-meta :description="'代表作 ' + item.presentBookName">
              ...
              <template #avatar
                ><a-avatar
                  :src="FetchImage(LIB_AUTHOR_AVATOR, item.authorInfo)"
              /></template>
            </a-list-item-meta>
          </a-list-item>
        </template>
      </a-list>
      <a-divider><h2>成员</h2></a-divider>
      <a-list
        item-layout="vertical"
        :pagination="memPagi"
...
const members = ref({});
const social = ref({});
const socialId = route.params.id;
...
function Init() {
  ...
  FetchSocialInfo(socialId, (data) => {
    social.value = data;
    members.value.master = {
      id: data.masterId,
      name: data.masterName,
      presentBookId: data.presentBookId,
